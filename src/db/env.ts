...
export async function getEnvInfoFn() {
  ...
  const ghLoggedIn = ghInstalled ? await checkCommand('gh auth status') : false
...
export async function getProjectScriptsFn(data: { repoPath: string }) {
...
export async function runHealthCheckFn(data: { command: string; cwd?: string }) {
  const cwd = data.cwd && existsSync(data.cwd) ? data.cwd : process.cwd()
  try {
    const { stdout, stderr } = await execAsync(data.command, { timeout: 30_000, cwd })
...
export const getEnvInfo = getEnvInfoFn
export const getProjectScripts = getProjectScriptsFn
export const runHealthCheck = runHealthCheckFn
